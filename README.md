...
The integration serves its own card and loads it on the frontend automatically (in both storage- and YAML-mode dashboards), so there is **no resource to add**. If the card doesn't show up right after install, hard-refresh the browser (Ctrl/Cmd+Shift+R) so it picks up the new frontend module. If you had added a manual resource for an older version under **Settings → Dashboards → ⋮ → Resources**, you can remove it - it's no longer needed (harmless if left).
...
That's it - there are no required options. Editing the card in the dashboard opens a small visual editor (Pulse and Full-bleed toggles) alongside the standard **Layout** and **Visibility** tabs. Panel name, slot count (default 40), and column count are edited live from the gear icon on the card. In a Sections dashboard the card defaults to **full width**; use the **Layout** tab to resize it.
