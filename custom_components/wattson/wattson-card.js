...
class WattsonCard extends HTMLElement {
  ...
  _inspectorHtml() {
    ...
    if (!total) {
      ...
      for (let i = 0; i < devs.length; i++) body += this._linkRow(this._deviceName(devs[i]), this._isDeviceActive(devs[i]), this._isManualDevice(devs[i]) ? "MANUAL" : "DEV", "device", devs[i]);
